...
test('searches and opens a working tool', async ({ page }) => {
  ...
  await page
    ...
    .fill('{"project":"Kits","status":"working"}')
...
test('formats JavaScript in the browser', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { name: 'JavaScript Formatter', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('is not available yet')).toHaveCount(0)
  await page
    .getByLabel('JavaScript input')
    .fill('function greet(name){return "hi "+name}')
  await page.getByRole('button', { name: 'Format' }).click()
  await expect(page.getByLabel('JavaScript result')).toHaveValue(
    /function greet\(name\) \{\n {2}return "hi " \+ name;\n\}/,
  )
  expect(errors).toEqual([])
})
...
test('formats HTML in the browser without executing it', async ({ page }) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'HTML Formatter', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('is not available yet')).toHaveCount(0)
  await page
    .getByLabel('HTML input')
    .fill(
      '<section><div><p>Hi</p><script>document.title="pwned"</script></div></section>',
    )
  await page.getByRole('button', { name: 'Format' }).click()
  await expect(page.getByLabel('HTML result')).toHaveValue(
...
test('opens html-to-image with HTML input', async ({ page }) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'HTML to Image', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('is not available yet')).toHaveCount(0)
...
test('opens favicon generator with a local dropzone', async ({ page }) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'Favicon Generator', level: 1 }),
  ).toBeVisible()
...
test('opens crop-image with crop controls', async ({ page }) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'Crop Image', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('Drop or paste your file here')).toBeVisible()
  expect(errors).toEqual([])
})
...
test('opens photo editor and exports a PNG', async ({ page }) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'Photo Editor', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('is not available yet')).toHaveCount(0)
  await expect(page.getByText('Drop or paste your file here')).toBeVisible()
...
test('opens a QR generator and a PDF workspace', async ({ page }) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'Merge PDF', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('Drop or paste your files here')).toBeVisible()
  expect(errors).toEqual([])
})

test('opens voice recorder and records with a mocked microphone', async ({
  page,
}) => {
  const errors: string[] = []
  page.on('pageerror', (error) => errors.push(error.message))
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'Voice Recorder', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('is not available yet')).toHaveCount(0)
  ...
  await expect(
    page.locator('audio[aria-label="Recording preview"]'),
  ).toBeVisible()
...
test('converts pasted SVG to a real PNG in the browser', async ({ page }) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'SVG to PNG', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('is not available yet')).toHaveCount(0)
  await page
    .getByLabel('SVG markup')
    .fill(
      '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="16"><rect width="32" height="16" fill="#00ff00"/></svg>',
    )
...
test('renders meme captions onto a downloaded PNG without uploading', async ({
  page,
}) => {
  const errors: string[] = []
  page.on('pageerror', (error) => {
  ...
  await waitForClient(page)
  await expect(
    page.getByRole('heading', { name: 'Meme Generator', level: 1 }),
  ).toBeVisible()
  await expect(page.getByText('is not available yet')).toHaveCount(0)
  const png = await page.evaluate(async () => {
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      canvas.toBlob(
        (next) =>
          next ? resolve(next) : reject(new Error('PNG encode failed.')),
        'image/png',
      )
    })
  ...
  await expect(
    page.getByRole('img', { name: 'Selected meme image' }),
  ).toBeVisible()
