...
async function waitForClient(page: Page) {
  await page.waitForFunction(() => {
    ...
    return hydrated && Boolean(window.localStorage.getItem('utility:installation-id'))
  })
}
...
async function activateOnPage(page: Page, licenseKey: string) {
  ...
  await expect(input).toHaveValue(licenseKey)
  const pending = page.waitForResponse(
    (response) =>
      response.request().method() === 'POST' && response.url().includes('/api/v1/licenses/activate'),
  )
...
function lockedHeading(page: Page) {
  return page.getByRole('heading', { name: 'Activate a Pro license to use this tool.' })
}
...
async function expectUnlocked(page: Page) {
  await expect(page.getByRole('heading', { name: 'Remove Background', level: 1 })).toBeVisible()
  await expectLockedHeading(page, 0)
  await expect(page.getByRole('link', { name: 'Pro', exact: true })).toBeVisible()
}

test.describe('licensing', () => {
  test('free tools stay open and one license maps to one installation', async ({ browser, page, request }) => {
    ...
    await waitForClient(page)
    await expectUnlocked(page)
    await expect(page.getByText(/Pro 1 Month is active on this device/)).toHaveCount(0)
    await expect(page.getByRole('link', { name: 'Pro', exact: true })).toBeVisible()
    ...
    await expect(page.getByText('Paste a Pro license key. No account is created.')).toBeVisible()
