...
import { type ImageCrop, type ImageFormat, type WatermarkPosition } from '@/processing/client/image'
import { rasterEncodeQuality, type ImageOptimizeMode } from '@/processing/client/image-optimize'
...
function RasterImageWorkspace({ tool }: { tool: ToolDefinition }) {
  ...
  async function run() {
    ...
    try {
      ...
      if (mode === 'favicon') {
        ...
        setProcessPercent(stagePercent('compressing'))
        await yieldToUi()
        const generated = await generateFaviconPack(file, { fit })
        if (selection !== selectionRef.current) return
        setProcessStage('finalizing')
        setProcessPercent(stagePercent('finalizing'))
        await yieldToUi()
        clearResult()
        const urls: string[] = []
  ...
  useGSAP(() => {
    ...
    const progress = workspaceRef.current?.querySelector('.processing-progress')
    const outcome = workspaceRef.current?.querySelector('.compress-result')
    const target = outcome ?? progress
    if (target) {
      gsap.fromTo(target, { autoAlpha: 0, y: 8 }, { autoAlpha: 1, y: 0, duration: motion.duration.instant, ease: motion.ease.enter })
    }
    ...
    gsap.fromTo(panel, { autoAlpha: 0, y: 20, scale: 0.98, filter: 'blur(8px)' }, {
      autoAlpha: 1, y: 0, scale: 1, filter: 'blur(0px)', duration: motion.duration.fast, ease: motion.ease.enter, clearProps: 'filter',
    })
