...
const labels = { overview: 'Overview', licenses: 'Licenses', audit: 'Audit log', system: 'System' }
...
function Overview() {
  ...
  const metrics = [
    ['Active licenses', query.data.active], ['Unused licenses', query.data.unused], ['Expiring soon', query.data.expiringSoon],
    ['Revoked licenses', query.data.revoked], ['Created this month', query.data.createdThisMonth],
  ]
  return <section><div className="admin-metrics">{metrics.map(([label, value]) => <article key={label}><span>{label}</span><strong>{value}</strong></article>)}</div><div className="admin-callout"><div><p className="eyebrow">Security posture</p><h2>Licenses stay opaque by default.</h2><p>Keys are shown once at creation. Stored digests, installation hashes, and signing secrets never enter this dashboard.</p></div><ShieldCheck size={34} /></div></section>
}
...
function Licenses() {
  ...
  const [confirm, setConfirm] = useState<{ license: AdminLicense; action: 'renew' | 'suspend' | 'resume' | 'revoke' | 'reset-activations' } | null>(null)
  const licenses = useQuery({ queryKey: ['admin', 'licenses', page, search, status], queryFn: () => listAdminLicenses({ page, pageSize: 25, search, ...(status ? { status } : {}) }) })
  const action = useMutation({ mutationFn: ({ id, action, duration }: { id: string; action: 'renew' | 'suspend' | 'resume' | 'revoke' | 'reset-activations'; duration?: DurationMonths }) => mutateAdminLicense(id, action, duration), onSuccess: async () => { setConfirm(null); await client.invalidateQueries({ queryKey: ['admin'] }) } })
  return <section>
    <div className="admin-toolbar"><form role="search" onSubmit={(event) => event.preventDefault()}><Search size={16} /><input aria-label="Search licenses" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1) }} placeholder="ID, key prefix, or note" /></form><select aria-label="Filter by status" value={status} onChange={(event) => { setStatus(event.target.value as AdminDisplayStatus | ''); setPage(1) }}><option value="">All statuses</option>{['unused', 'active', 'suspended', 'expired', 'revoked'].map((item) => <option key={item}>{item}</option>)}</select><button className="button primary" type="button" onClick={() => setCreateOpen(true)}><Plus size={16} />Create license</button></div>
    {licenses.isError && <p className="admin-error" role="alert">{errorMessage(licenses.error)}</p>}
    <div className="admin-table-wrap"><table><thead><tr><th scope="col">License</th><th scope="col">Plan</th><th scope="col">Status</th><th scope="col">Created</th><th scope="col">Activated</th><th scope="col">Expires</th><th scope="col"><span className="sr-only">Actions</span></th></tr></thead><tbody>{licenses.data?.items.map((license) => <LicenseRow key={license.licenseId} license={license} onAction={(next) => setConfirm({ license, action: next })} />)}{licenses.data?.items.length === 0 && <tr><td colSpan={7} className="admin-empty-row">No licenses match these filters.</td></tr>}</tbody></table></div>
    {licenses.data && <div className="admin-pagination"><span>{licenses.data.total} licenses</span><div><button className="button ghost" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Previous</button><span>Page {page} of {licenses.data.pages}</span><button className="button ghost" disabled={page >= licenses.data.pages} onClick={() => setPage((value) => value + 1)}>Next</button></div></div>}
    <CreateDialog open={createOpen} issued={issued} onIssued={setIssued} onClose={() => { setCreateOpen(false); setIssued(null); void client.invalidateQueries({ queryKey: ['admin'] }) }} />
    <ConfirmDialog value={confirm} pending={action.isPending} error={action.error} onClose={() => setConfirm(null)} onConfirm={(duration) => { if (confirm) action.mutate({ id: confirm.license.licenseId, action: confirm.action, ...(duration ? { duration } : {}) }) }} />
  </section>
}
...
function LicenseRow({ license, onAction }: { license: AdminLicense; onAction: (action: 'renew' | 'suspend' | 'resume' | 'revoke' | 'reset-activations') => void }) {
  ...
  return <tr><th scope="row"><strong>{license.keyPrefix ?? 'KITS'}</strong><small>{license.licenseId.slice(0, 8)}</small></th><td>{license.plan.replace('pro_', '').replace('_months', ' months').replace('_month', ' month')}</td><td><span className={`admin-status ${state}`}>{state}</span></td><td><time dateTime={license.createdAt}>{date(license.createdAt)}</time></td><td>{date(license.activatedAt)}</td><td>{date(license.expiresAt)}</td><td><div className="admin-row-actions">{license.status !== 'revoked' && <button onClick={() => onAction('renew')}>Renew</button>}{license.status === 'suspended' ? <button onClick={() => onAction('resume')}>Resume</button> : license.status === 'active' && <button onClick={() => onAction('suspend')}>Suspend</button>}{license.installationActive && <button onClick={() => onAction('reset-activations')}>Reset</button>}{license.status !== 'revoked' && <button className="danger" onClick={() => onAction('revoke')}>Revoke</button>}</div></td></tr>
}
...
function ConfirmDialog({ value, pending, error, onClose, onConfirm }: { value: { license: AdminLicense; action: string } | null; pending: boolean; error: unknown; onClose: () => void; onConfirm: (duration?: DurationMonths) => void }) {
  const dialog = useRef<HTMLDialogElement>(null)
  const [duration, setDuration] = useState<DurationMonths>(1)
  useEffect(() => { if (value && !dialog.current?.open) dialog.current?.showModal(); if (!value && dialog.current?.open) dialog.current.close() }, [value])
  if (!value) return <dialog ref={dialog} />
  const destructive = value.action === 'revoke'
  const title = `${value.action.replace('-', ' ')} this license?`
  return <dialog ref={dialog} className="admin-dialog" aria-labelledby="confirm-title" onCancel={(event) => { event.preventDefault(); onClose() }}><div><p className="eyebrow">Confirm action</p><h2 id="confirm-title">{title}</h2><p>{destructive ? 'This immediately removes Pro access and cannot be reversed.' : value.action === 'reset-activations' ? 'The license can activate on another browser or device afterward.' : 'The backend will verify that this state transition is allowed.'}</p>{value.action === 'renew' && <label>Extend by<select value={duration} onChange={(event) => setDuration(Number(event.target.value) as DurationMonths)}><option value={1}>1 month</option><option value={6}>6 months</option><option value={12}>12 months</option></select></label>}{error !== null && error !== undefined ? <p className="admin-error" role="alert">{errorMessage(error)}</p> : null}<div className="button-row"><button className="button ghost" onClick={onClose}>Cancel</button><button className={`button ${destructive ? 'danger-button' : 'primary'}`} disabled={pending} onClick={() => onConfirm(value.action === 'renew' ? duration : undefined)}>{pending ? 'Working...' : `Confirm ${value.action.replace('-', ' ')}`}</button></div></div></dialog>
}
