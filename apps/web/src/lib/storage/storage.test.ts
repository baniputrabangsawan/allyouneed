...
import {
  ...
  getDeviceSecret,
  getOrCreateInstallationCredentials,
...
describe("storage helpers", () => {
  ...
  it("keeps at most ten unique recent tools with the newest first", () => {
    ...
    expect(getRecentTools(storage)).toEqual([
      "tool-5",
      "tool-0",
      "tool-1",
      "tool-2",
  ...
  it("parses recent tool ids from the SSR cookie header", () => {
    expect(
      parseRecentCookie(
        "kits_recent=json-formatter%7Ccompress-image; theme=dark",
      ),
    ).toEqual(["json-formatter", "compress-image"]);
    expect(parseRecentCookie("")).toEqual([]);
    expect(parseRecentCookie("kits_recent=bad id|ok-tool")).toEqual([
      "ok-tool",
    ]);
  });

  it("parses favorite ids from the SSR cookie header", () => {
    expect(
      parseFavoritesCookie(
        "kits_favorites=compress-image%7Cjson-formatter; kits_recent=x",
      ),
    ).toEqual(["compress-image", "json-formatter"]);
  ...
  it("creates and reuses secure installation credentials", () => {
    const storage = memoryStorage();
    const first = getOrCreateInstallationCredentials(storage);
    const second = getOrCreateInstallationCredentials(storage);
    expect(first?.installationId).toMatch(/^[0-9a-f-]{36}$/i);
    expect(first?.deviceSecret).toMatch(/^[A-Za-z0-9_-]{43}$/);
    expect(second).toEqual(first);
    expect(getDeviceSecret(storage)).toBe(first?.deviceSecret);
    storage.setItem(DEVICE_SECRET_STORAGE_KEY, "not-a-secret");
    expect(getDeviceSecret(storage)).toBeUndefined();
  });
