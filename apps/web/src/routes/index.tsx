...
import { RECENT_TOOLS_STORAGE_KEY } from '@/lib/storage/tools'
...
export const Route = createFileRoute('/')({ validateSearch: searchSchema, component: Home })
...
function getRecentSnapshot() {
  try { return localStorage.getItem(RECENT_TOOLS_STORAGE_KEY) ?? '[]' } catch { return '[]' }
}

function useRecentIds() {
  const snapshot = useSyncExternalStore(subscribeToDiscovery, getRecentSnapshot, () => '[]')
  try {
  ...
  return <main>
    <section className="hero discovery-hero">
      <div className="hero-grid">
        <div><p className="eyebrow"><Sparkles size={14}/> Useful by default</p><h1>Every tool you need.<br/><span>In one place.</span></h1><p className="hero-copy">Compress, convert, edit, and generate directly in your browser. Fast, private, and free to start.</p></div>
    ...
    {!q && category === 'all' && group === 'all' && <>
      ...
      <section className="page-section popular"><div className="section-heading"><div><p className="eyebrow">Start here</p><h2>Popular tools</h2></div><a href="#all-tools">Browse all <ArrowRight size={16}/></a></div><ToolGrid items={getPopularTools()}/></section>
    </>}
...
function ToolGrid({ items }: { items: readonly ToolDefinition[] }) {
  return <div className="tool-grid">{items.map((tool) => <DiscoveryToolCard key={tool.id} tool={tool}/>)}</div>
}
...
function ToolSection({ id, eyebrow, title, icon, items, empty }: { id: string; eyebrow: string; title: string; icon: ReactNode; items: readonly ToolDefinition[]; empty: string }) {
  return <section id={id} className="page-section discovery-personal"><div className="section-heading"><div><p className="eyebrow">{icon}{eyebrow}</p><h2>{title}</h2></div></div>{items.length ? <ToolGrid items={items}/> : <p>{empty}</p>}</section>
}
