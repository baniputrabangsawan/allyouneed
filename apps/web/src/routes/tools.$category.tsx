...
function CategoryCatalog() {
  ...
  return <main className="catalog-page"><header className="catalog-header"><nav className="breadcrumb" aria-label="Breadcrumb"><Link to="/">Home</Link><ChevronRight size={14}/><Link to="/tools">Tools</Link><ChevronRight size={14}/><span>{category}</span></nav><p className="eyebrow">Browse by category</p><h1>{category} tools</h1><p>{categoryTools.length} utilities for {category === 'converter' ? 'moving between formats' : `working with ${category}`}.</p><div className="catalog-categories" aria-label="Other categories">{validCategories.map((item) => <Link key={item} className={item === category ? 'active' : ''} aria-current={item === category ? 'page' : undefined} to="/tools/$category" params={{ category: item }}>{item}</Link>)}</div></header><section className="page-section catalog-grid-section"><div className="section-heading"><div><p className="eyebrow">{categoryTools.filter((tool) => tool.available).length} available now</p><h2>All {category} tools</h2></div></div><div className="tool-grid">{categoryTools.map((tool) => <DiscoveryToolCard key={tool.id} tool={tool}/>)}</div></section></main>
}
