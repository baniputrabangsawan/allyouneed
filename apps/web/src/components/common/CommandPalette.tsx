...
import { getPopularTools, getToolBySlug, type ToolDefinition } from '@/features/tools/tool-registry'
...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const defaults: PaletteTool[] = [
    ...
    ...getPopularTools()
      .filter((tool) => !recent.some(({ id }) => id === tool.id))
      .map((tool) => ({ ...tool, source: 'Popular' as const })),
  ]
  ...
  const items: PaletteItem[] = [
    ...tools.map((tool) => {
      ...
      return {
        ...
        id: tool.id,
        title: item.name,
        subtitle: copy.category[tool.category] ?? tool.category,
        slug: tool.slug,
    ...
    ...(!query
      ...
      : searchDocs(query)
          ...
          .map((hit) => ({
            ...
            title: docsResultTitle(hit, locale, copy.palette.howTo),
    ...
    >
      ...
      >
        ...
        <label className="command-search">
          <Search size={20} />
          <span className="sr-only">{copy.palette.title}</span>
          <input
            ref={inputRef}
            role="combobox"
            aria-expanded="true"
            aria-controls="command-results"
            aria-activedescendant={items[activeIndex] ? `command-result-${activeIndex}` : undefined}
            value={query}
            onChange={(event) => {
        ...
        <div ref={resultsRef} id="command-results" className="command-results" role="listbox" aria-label={query ? copy.home.matchingTools : copy.palette.popular}>
          {items.map((item, index) => {
            ...
            return (
              <div key={item.id}>
                {section && section !== previousSection && <p className="command-section-label">{section}</p>}
                <button id={`command-result-${index}`} data-result-index={index} role="option" aria-selected={index === activeIndex} className={index === activeIndex ? 'active' : ''} type="button" onMouseEnter={() => setActiveIndex(index)} onClick={() => choose(item)}>
                  <Icon size={17} />
        ...
        <div className="command-footer">
          <span>
            <kbd>↑</kbd>
            <kbd>↓</kbd> {copy.palette.navigate}
          </span>
          <span>
            <kbd>↵</kbd> {copy.palette.open}
          </span>
          <span>{copy.palette.searchAll}</span>
        </div>
      </div>
    </dialog>
  )
...
function docsResultTitle(hit: ReturnType<typeof searchDocs>[number], locale: ReturnType<typeof useLocale>, howTo: (toolName: string) => string): string {
  if (hit.kind === 'article') return hit.title
  const tool = getToolBySlug(hit.slug)
  return tool ? howTo(localizeTool(tool, locale).name) : hit.title
}
